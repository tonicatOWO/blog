...
// Pull all string literals out of JS-ish code so chars in frontmatter / <script> /
// .ts / .json survive.
		...
		await Promise.all(
			entries.map(async entry => {
				...
				if (entry.isDirectory()) {
					...
					for (const c of extractText(content, extname(entry.name)))
						chars.add(c);
				}
			}),
		);
...
async function subsetFontFile(fontPath, chars) {
	...
	await writeFile(
		join(SUBSET_DIR, `${basename(fontPath, extname(fontPath))}.subset.woff2`),
		output
	);
	const saved = ((1 - output.byteLength / input.byteLength) * 100).toFixed(1);
	console.log(
		`  ✓ ${basename(fontPath)} ${(input.byteLength / 1024).toFixed(1)}KB → ` +
		`${(output.byteLength / 1024).toFixed(1)}KB (-${saved}%)`
	);
}

// Recursively collect files we need to patch (HTML, CSS, JS — anywhere
// @font-face or font-loader code may live).
...
async function patchAssetUrls() {
	...
	for (const fp of files) {
		...
		updated = updated.replace(
			/(<link\b(?=[^>]*\brel=(["'])preload\2)(?=[^>]*\bas=(["'])font\3)[^>]*?)(\/?>)/gi,
			(match, tag, _q1, _q2, close) =>
...
const subsetFontsExist = (
	await Promise.all(
		fonts.map(fp =>
			readFile(
				join(SUBSET_DIR, `${basename(fp, extname(fp))}.subset.woff2`)
			).then(
				() => true,
				() => false
			)
...
const cacheHit =
	subsetFontsExist &&
	existsSync(cacheKeyFile) &&
	(await readFile(cacheKeyFile, 'utf-8')) === cacheKey;

// Drop orphaned subsets (e.g. Astro's content-hashed font filenames change
// between builds).
const expectedSubsets = new Set(
	fonts.map(fp => `${basename(fp, extname(fp))}.subset.woff2`)
);
const existingSubsets = existsSync(SUBSET_DIR)
	? await readdir(SUBSET_DIR)
	: [];
for (const name of existingSubsets) {
	if (!expectedSubsets.has(name))
		await unlink(join(SUBSET_DIR, name));
}
...
if (cacheHit) {
	...
	if (failures.length > 0) {
		for (const { fp, err } of failures)
			console.error(`  ✗ Failed: ${basename(fp)} - ${err.message}`);
...
if (fonts.length > 0) {
	console.log(
		`  Removed ${fonts.length} original font(s) ` +
